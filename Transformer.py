...
class AFF(tf.keras.layers.Layer):
    def __init__(self,channels=64,r=4):
        ...
        self.l_C1=tf.keras.layers.Conv1D(inter_channels,kernel_size=1)
        self.l_C2=tf.keras.layers.Conv1D(channels,kernel_size=1)
        self.g_C1=tf.keras.layers.Conv1D(inter_channels,kernel_size=1)
        self.g_C2=tf.keras.layers.Conv1D(channels,kernel_size=1)
    ...
    def call(self,x,residual):
        ...
        xl=tf.keras.layers.ReLU()(xl)
        xl=self.l_C2(xl)
        xl=tf.keras.layers.BatchNormalization()(xl)
        
        xg=tf.keras.layers.AvgPool1D()(xa)
        xg=self.g_C1(xa)
        xg=tf.keras.layers.BatchNormalization()(xg)
        xg=tf.keras.layers.ReLU()(xg)
        xg=self.g_C2(xg)
        xg=tf.keras.layers.BatchNormalization()(xg)
        xlg = xl + xg
        wei = tf.sigmoid(xlg)
...
class EncoderModel(tf.keras.layers.Layer):
    def __init__(self,num_layers,max_length,d_model,num_heads,dff,rate=0.1):
        ...
        self.weight = feed_forward_network(d_model,dff)
